...
def confirm_keyboard(plan: str):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💳 Proceed to Payment", callback_data=f"buy_{plan}")],
...
async def choose_lite(callback: CallbackQuery):
    await callback.message.edit_text(
        ...
        "• Lifetime updates\n\n"
        "Proceed to payment?",
        reply_markup=confirm_keyboard("google")  # твій product_key
    )
...
async def choose_pro(callback: CallbackQuery):
    await callback.message.edit_text(
        ...
        "• Lifetime updates\n\n"
        "Proceed to payment?",
        reply_markup=confirm_keyboard("meta")  # твій product_key
    )
